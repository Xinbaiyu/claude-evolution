# analysis-status-api Specification

## Purpose
提供实时分析运行状态查询接口，支持前端在页面刷新后恢复 loading 状态。通过 `GET /api/analyze/status` 端点返回当前分析运行的元数据（是否运行中、开始时间、runId），前端据此恢复 UI 状态。

## Requirements

### Requirement: Analysis status query endpoint

系统 SHALL 提供 `GET /api/analyze/status` 端点，返回当前分析运行的实时状态信息。该端点 SHALL 从后端 `isAnalyzing` 标志和内存中读取状态，不依赖任何前端状态。
