...
系统采用双池架构，基于置信度和提及次数进行自动提升：

### Active Pool (活跃池)
- **容量**: 50 条观察（可配置）
- **作用**: 新提取的观察首先进入 Active Pool
- **衰减**: 30 天半衰期，置信度随时间降低

### Context Pool (上下文池)
- **容量**: 50 条高质量观察（可配置）
- **晋升条件**:
  - 置信度 ≥ 0.90 且提及次数 ≥ 10（自动晋升）
  - 置信度 ≥ 0.75 且提及次数 ≥ 5（高质量候选）
- **作用**: 写入 CLAUDE.md，作为持久化上下文

### 自动提升流程
1. 分析提取新观察 → Active Pool
2. 相似观察合并，提及次数累加，置信度提升
3. 达到晋升阈值 → 自动晋升到 Context Pool
4. 低质量观察衰减 → 删除或归档
